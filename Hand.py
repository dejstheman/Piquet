...
class Hand:
    ...
    def get_elder_discard_cards(self, n):
        aces = [card for card in self.cards if card.rank == 8]
        ...
        point_set_seq_aces = sorted([card for card in self.points if any(True for ace in aces if ace.suit == card.suit)
